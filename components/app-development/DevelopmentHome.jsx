...
// components/AppDevelopmentHome.jsx
import Image from "next/image";
import love from "../../public/home-image/love.svg"; // 🔁 path ta nijer structure onujayi change korba
  ...
  return (
    <div>
      ...
      <main className="flex items-center">
        ...
        <section
          className="relative flex w-full items-center bg-no-repeat bg-left bg-contain"
          style={{ backgroundImage: `url(${love.src})` }} // ✅ background ekhanei fixed
        >
          {/* app-container er moddhe sob content */}
          <div className="app-container flex flex-col gap-12 py-10 lg:flex-row lg:items-center lg:justify-between">
            {/* LEFT CONTENT */}
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {services.map((service, index) => (
                <article
                  ...
                  className="flex items-center gap-4 rounded-md bg-white/95 px-8 py-4 lg:py-6 shadow-[0_18px_45px_rgba(15,23,42,0.08)] ring-1 ring-slate-100 transition transform hover:-translate-y-1"
                >
                  {/* Green check icon */}
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#93c01f]">
                    <svg
                      viewBox="0 0 20 20"
                      aria-hidden="true"
                      className="h-5 w-5 text-white"
